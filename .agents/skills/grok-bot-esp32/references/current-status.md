...
- **Screensaver:** with no sessions, or 20 s after the hub's 5 s heartbeat stops, the board shows a silent screensaver that drifts through moods and head shapes and dims for the battery. It is vertically centered when no name band shows.
- **Tilt:** reacts to orientation changes only, using the measured IMU axes (+x toward the lanyard).
