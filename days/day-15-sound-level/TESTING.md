# Day 15 testing model

Run the generalized contract validation:

```sh
scripts/prompt-contract/validate_day.sh days/day-15-sound-level
```
...
For a generated device candidate, add domain tests for every numerical or protocol rule in SPEC.md, compile with pinned libraries, run the D15_ serial acceptance protocol, capture the declared reference state, and complete [HAND-REVIEW.md](HAND-REVIEW.md) on the attached Stopwatch.
