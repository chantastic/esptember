...
firmware: /firmware/day-15-sound-level.bin
...
Day 09 sent audio out; today audio comes in.
...
Download [day-15-sound-level.bin](https://esptember.com/firmware/day-15-sound-level.bin) and open a terminal in the download directory.
...
uvx esptool --chip esp32s3 --port PORT \
  write-flash 0x0 day-15-sound-level.bin
```
...
The mic path mirrors day 09's speaker path — same codec, opposite direction: init I2S, init the microphone codec, read PCM.
...
Same separation as day 09: audio never runs on the UI task, in either direction.
...
```sh
cd days/day-15-sound-level/firmware
idf.py build
