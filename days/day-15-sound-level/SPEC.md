# Day 15: Sound Level (dB) behavior contract
...
- `focus`: enum, default `peak`
- `integration`: enum, default `medium`
- `dbfs`: int, default `-96`
- `peak`: int, default `-96`
...
1. Capture signed PCM, remove DC, compute RMS, and convert to dBFS with a named silence floor.
2. Smooth the live level separately from the peak. Hold peak for three seconds, then release it toward live level.
3. Focus Reset Peak and Integration. Enter resets peak or cycles Fast, Medium, and Slow smoothing.
4. Show dBFS everywhere; never label the value dB SPL without a physical calibration procedure.
5. Reject NaN, infinity, clipped buffers, and zero-length input without corrupting the display state.

## Required scenarios

- **primary flow**: {'type': 'sample', 'dbfs': -42}, {'type': 'sample', 'dbfs': -55}, {'type': 'peak_timeout'}
- **reset peak**: enter
- **integration cycle**: right, enter, enter
