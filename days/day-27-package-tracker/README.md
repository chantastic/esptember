...
summary: "A provider-neutral package board with explicit status mapping, cached events, and secret-safe setup."
...
The durable source is this prompt, [SPEC.md](https://github.com/chantastic/esptember/blob/main/days/day-27-package-tracker/SPEC.md), the machine-readable contract, and its layered acceptance criteria.
Generated firmware is a disposable candidate.

## Reference frame

![Round-screen acceptance reference for Package Tracker](https://esptember.com/images/day-27-package-tracker/reference.png)
...
```text
Build ESPtember Day 27: Package Tracker for the M5Stack Stopwatch Dev Kit (C152).
...
- days/day-27-package-tracker/SPEC.md
- days/day-27-package-tracker/tests/contract.json
...
Run scripts/prompt-contract/validate_day.sh days/day-27-package-tracker before compiling a device candidate.
...
- Store no API key in source, binary, screenshots, or logs. Provision it at runtime and persist it separately.
- Keep provider registration/fetch/normalization behind one interface and map states to pre-transit, transit, delivery, delivered, exception.
- Left and right choose packages; Enter opens details or refreshes; Back returns to the list.
- Cache latest status and event atomically, retain it across outages, and mark age and stale state.
- Bound package count, response size, event text, request time, refresh rate, and retries.

Diagnostics and acceptance

- Prefix serial protocol lines with D27_ and implement status, reset, action, capture, and touchlog commands.
