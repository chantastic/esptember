# Day 18: Pokedex behavior contract
...
1. Ship exactly 151 entries with id, name, types, height, weight, four stats, and a bounded sprite asset.
2. Left and right move one entry with wrap; Enter opens detail; Back returns to the same list position.
3. Touch rows and Back share the same reducer as buttons. Fast navigation must not allocate unbounded memory.
4. Decode only the current sprite and release or reuse its buffer before loading another.
5. Provide prefix search through a focused on-screen filter without requiring a network.

## Required scenarios

- **primary flow**: right, enter, back
- **wrap**: left
- **search**: {'type': 'search', 'query': 'pika'}, enter
