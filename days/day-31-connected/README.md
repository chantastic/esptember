...
firmware: /firmware/day-31-connected.bin
...
September doesn't have a 31st, but the series had unfinished business: day 30 signed in and kept nothing.
...
- **A WorkOS application** with the Device Authorization Grant enabled (day 30's setup); its public `client_01...` id.
...
Download [day-31-connected.bin](https://esptember.com/firmware/day-31-connected.bin) and flash it at `0x0`:

```sh
uvx esptool --chip esp32s3 --port PORT \
  write-flash 0x0 day-31-connected.bin
```
...
```sh
cd days/day-31-connected/firmware
idf.py build
...
The sequel gives this resident identity something to do: day 32 turns it into your connected accounts, live on the same screen.
