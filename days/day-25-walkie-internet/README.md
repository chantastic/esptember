...
firmware: /firmware/day-25-walkie-internet.bin
...
Day 24 with one organ transplanted: the transport.
...
[day-25-walkie-internet.bin](https://esptember.com/firmware/day-25-walkie-internet.bin) (StopWatch TX) and
[day-25-walkie-internet-waveshare.bin](https://esptember.com/firmware/day-25-walkie-internet-waveshare.bin) (AMOLED 1.8 RX).

Flash each at `0x0`:

```sh
uvx esptool --chip esp32s3 --port STOPWATCH_PORT \
  write-flash 0x0 day-25-walkie-internet.bin
uvx esptool --chip esp32s3 --port WAVESHARE_PORT \
  write-flash 0x0 day-25-walkie-internet-waveshare.bin
```
...
Provisioning shows the day-21 rule paying off: the RX lifts `portal.c` verbatim, the TX reuses day 23's NVS namespace — provision once, every lesson benefits.
...
cd esptember/days/day-25-walkie-internet
```
...
TX half: `./scripts/build-stopwatch.sh`, upload as in day 24.
RX half: `cd firmware/waveshare && idf.py build flash`.
Point both at your relay by changing `RELAY_HOST` / `RELAY_URI_FMT`.

Day 24 ended promising this lesson; the pair is the series' clearest argument that **transports are swappable when the frame is the contract**.
