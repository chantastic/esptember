# Day 23: Weather Station behavior contract
...
- `screen`: enum, default `current`
- `focus`: enum, default `refresh`
- `temp`: int, default `0`
...
1. Use Open-Meteo over TLS with explicit latitude, longitude, timezone, units, and response limits.
2. Show current temperature, condition, high/low, wind, age of data, and a 12-hour bezel forecast.
3. Left and right move Current, Hourly, and Settings; Enter refreshes or edits; Back returns to Current.
4. Cache the last valid response and mark it stale instead of blanking the screen on a failed refresh.
5. Filter JSON fields, validate ranges and timestamps, and use named HTTP, parse, and clock errors.

## Required scenarios

- **primary flow**: {'type': 'weather', 'temp': 68}, {'type': 'minute'}
- **screen navigation**: right, right, back
- **stale cache**: {'type': 'refresh_failed'}, back
