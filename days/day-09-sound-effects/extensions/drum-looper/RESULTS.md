...
Each passed 351,390 assertions covering defaults, bounds, deterministic nonmutation, play/pause/resume, the audible-grid state, valid and stale tap-tempo input, independently anchored 64-step replacement windows, same-step hit collapse, paused audition, loop wrap, replacement completion and restart, reset, Stopwatch metadata, round-screen layout, and 10,000 generated actions.
...
The corrected four-measure application used 916,855 bytes of its 3,145,728-byte slot and 31,584 bytes of static RAM.
...
- 100 live audio starts at 35 µs p95 and 886 µs maximum request-to-accepted latency;
- an empty 120 BPM loop advancing exactly 96 sixteenth steps, completing one four-measure pass, and scheduling exactly 24 quarter-note clicks in 12 seconds, with zero late steps;
- one real-time four-measure pass scheduling 17 clicks through its four-step observation tail, with zero late steps;
