# Day 28: Real-Time Captions behavior contract
...
- `status`: enum, default `connecting`
- `size`: enum, default `medium`
- `finals`: int, default `0`
- `interim`: string, default ``
...
1. Provision Wi-Fi and the transcription key at runtime; redact the key everywhere.
2. Stream 16 kHz mono linear PCM in bounded chunks and keep audio capture independent from network writes.
3. Replace the interim line in place; append finalized lines exactly once and retain the newest six.
4. Enter pauses/resumes capture; Back clears transcript only after confirmation; left/right change text size.
5. Handle reconnect, keepalive, malformed JSON, empty alternatives, and service errors without losing finalized text.

## Required scenarios

- **primary flow**: {'type': 'connected'}, {'type': 'interim', 'text': 'hello wor'}, {'type': 'final', 'text': 'Hello world.'}
- **pause**: enter, enter
- **text size**: right, right
