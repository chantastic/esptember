// ESPtember Day 24 — Internet Walkie-Talkie, receiver half (Waveshare 1.8)
// Day 23's receiver with the transport swapped: voice frames arrive as
...
// squelch — untouched. Provisioning is day 21's captive portal, lifted
...
// --- Jitter buffer (day 23, unchanged) ------------------------------------
...
static void audio_task(void *arg)
{
    ...
    esp_codec_dev_sample_info_t fs = {
        .sample_rate = 2 * SAMPLE_RATE, // 2x upsample, as day 23
        .channel = 1,
        .bits_per_sample = 16,
    };
    ...
    while (1) {
        ...
        if (flowing && xQueueReceive(audio_queue, &frame, pdMS_TO_TICKS(30))) {
            ...
            vTaskDelay(1); // >= 1 tick, per day 23's watchdog lesson
        }
    }
}
...
void app_main(void)
{
    ...
    if (!portal_load_creds(ssid, sizeof(ssid), pass, sizeof(pass))) {
        // No Wi-Fi yet: become the setup page (day 21, verbatim).
        bsp_display_lock(0);
