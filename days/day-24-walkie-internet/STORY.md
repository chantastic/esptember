...
The frame struct, the mic pacing, the jitter buffer, the squelch, the channel dial, the screens — byte-for-byte day 23.

That's the lesson working as designed: when the *frame* is the contract, the carrier underneath is an implementation detail.
Day 20 proved it across vendors; day 24 proves it across a planet.
...
It didn't.
Internet jitter and RF jitter are the same disease — frames arrive unevenly, the speaker consumes evenly — and day 23's 60 ms prebuffer treats both without modification.
...
This is the first day that *needed* day 21 and day 22 simultaneously — two boards, two provisioning styles, one rule.
The RX lifts `portal.c` verbatim; the TX reads day 22's NVS namespace so a board provisioned for weather is already provisioned for voice.
