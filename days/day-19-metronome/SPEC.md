# Day 19: Metronome behavior contract
...
1. BtnA starts and stops. BtnB taps tempo; use a robust median of recent intervals and ignore implausible gaps.
2. Touch or a focused settings row changes 40–240 BPM, meter 2/4 through 6/8, sound, and vibration.
3. Schedule beats from an absolute next-deadline sequence so rendering and feedback never accumulate drift.
4. Accent beat one and keep the visual pendulum phase tied to the same clock.
5. Changing tempo while running takes effect on the next beat without double-firing.

## Required scenarios

- **primary flow**: left, {'type': 'beat'}, left
- **tap tempo**: {'type': 'tap', 'interval_ms': 500}, {'type': 'tap', 'interval_ms': 600}
- **tempo bounds**: {'type': 'set_bpm', 'value': 500}, {'type': 'set_bpm', 'value': 10}
