...
summary: "Twenty-seven workouts, durable progress, exact interval accounting, and the project's reference controls."
verification: "Prompt contract and reference frame verified; generated hardware candidate awaits hand review"
---

## The assignment

Build the complete Couch-to-5K trainer that defines the project's default control grammar.

The durable source is this prompt, [SPEC.md](https://github.com/chantastic/esptember/blob/main/days/day-13-c25k/SPEC.md), the machine-readable contract, and its layered acceptance criteria.
Generated firmware is a disposable candidate.

## Reference frame

![Round-screen acceptance reference for Couch to 5K](https://esptember.com/images/day-13-c25k/reference.png)
...
The earlier instrumented Stopwatch build also produced real framebuffer captures:

| Running | Paused |
| --- | --- |
| ![C25K running interval on the Stopwatch](https://esptember.com/images/day-13-c25k/run.png) | ![C25K paused countdown on the Stopwatch](https://esptember.com/images/day-13-c25k/paused.png) |
| **Workout summary** | **Saved history** |
| ![Completed workout summary](https://esptember.com/images/day-13-c25k/workout-summary.png) | ![C25K history counts](https://esptember.com/images/day-13-c25k/history-counts.png) |

## The build prompt

```text
Build ESPtember Day 13: Couch to 5K for the M5Stack Stopwatch Dev Kit (C152).
...
- days/day-13-c25k/SPEC.md
- days/day-13-c25k/tests/contract.json
...
Run scripts/prompt-contract/validate_day.sh days/day-13-c25k before compiling a device candidate.
...
- Include all 27 workouts and validate every segment duration and total before startup.
- Browse workouts without moving the saved cursor; Enter starts the visible workout.
- Account elapsed time across multiple interval boundaries and assign each millisecond to the correct phase.
- Pause, resume, restart or go back, confirm cancellation, and preserve partial attempts honestly.
- Persist history, preferences, RTC setup, and the next-workout cursor with verifiable writes.

Diagnostics and acceptance

- Prefix serial protocol lines with D13_ and implement status, reset, action, capture, and touchlog commands.
