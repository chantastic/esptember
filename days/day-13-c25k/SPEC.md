...
M5Unified mapping, with the lanyard loop at the bottom: `M5.BtnA` = **left** (Yellow), `M5.BtnB` = **right** (Blue).
Power button is handled by M5PM1 and left alone.
...
|---|---|---|
| Right / Blue press | **Next** | BtnB `wasClicked()` with BtnA not held |
| Left / Yellow press | **Prev** | BtnA `wasClicked()` with BtnB not held |
...
| 1.3 | 2026-09-23 | Clarify the physical mapping: BtnA is left, BtnB is right, with the lanyard down. No firmware behavior or storage change. |
