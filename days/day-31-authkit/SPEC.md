# Day 31: WorkOS AuthKit behavior contract
...
1. Provision only the public client id; never ship a client secret or WorkOS API key to the device.
2. Request a device code, show a scannable QR plus human code, and poll at the server-provided interval.
3. Handle authorization_pending, slow_down, access_denied, expired_token, network failure, and success by name.
4. Left/right focus Retry and Cancel; Enter acts; Back cancels polling and returns to setup.
5. Redact tokens from logs and captures; display only safe identity claims after signature/trust checks appropriate to the flow.

## Required scenarios

- **primary flow**: {'type': 'code', 'value': 'ABCD-EFGH'}, {'type': 'poll'}, {'type': 'authorized', 'email': 'user@example.com'}
- **slow down**: {'type': 'poll'}, {'type': 'slow_down'}
- **cancel**: back
