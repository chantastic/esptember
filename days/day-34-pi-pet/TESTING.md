...
- **Replay:** `python3 tests/check_device.py` (needs pyserial) replays each scenario without a `given` state or a 30 s / 10 min clock. It checks the device's `pets` report and its emitted `GB_FOCUS`/`GB_ACK` lines after every step. Waits stand in for `advance_5s`.
