...
firmware: /firmware/day-27-captions.bin
...
Download [day-27-captions.bin](https://esptember.com/firmware/day-27-captions.bin) and flash it at `0x0`:

```sh
uvx esptool --chip esp32s3 --port PORT \
  write-flash 0x0 day-27-captions.bin
```

Wi-Fi via day 21's portal, then the key over serial:
...
Day 14's mic capture reads 200 ms chunks; day 24's WebSocket discipline carries them; Deepgram's live endpoint takes raw PCM as binary messages, no framing, no base64:
...
The mic paces the stream, exactly as it paced day 23's walkie-talkie: each blocking read fills a chunk, each chunk sends whole.
...
```sh
cd days/day-27-captions/firmware
idf.py build
